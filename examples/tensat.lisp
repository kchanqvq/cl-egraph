...
(defrw2 conv-is-bilinear-1 (smul (conv2d ?sx ?sy ?p none ?x ?y) ?w) (conv2d ?sx ?sy ?p none (smul ?x ?w) ?y))
(defrw2 conv-is-bilinear-2 (conv2d ?sx ?sy ?p none ?x (ewadd ?y ?z)) (ewadd (conv2d ?sx ?sy ?p none ?x ?y) (conv2d ?sx ?sy ?p 0 ?x ?z)))
(defrw2 conv-is-bilinear-3 (conv2d ?sx ?sy ?p none (ewadd ?x ?y) ?z) (ewadd (conv2d ?sx ?sy ?p none ?x ?z) (conv2d ?sx ?sy ?p 0 ?y ?z)))
(defrw -enlarge-convolution-kernel (conv2d ?sx ?sy none ?c ?x (enlarge ?kx ?ky ?y)) (conv2d ?sx ?sy none ?c ?x ?y))
(defrw2 operator-commutativity-4 (conv2d ?sx ?sy ?p relu ?x ?y) (relu (conv2d ?sx ?sy ?p none ?x ?y)))
(defrw2 conv-with-2-applies-relu (relu (transpose ?x)) (transpose (relu ?x)))
(defrw -pooling-by-conv.-with-cpool (poolavg ?kx ?ky ?sx ?sy ?p ?x) (conv2d ?sx ?sy ?p none ?x (cpool ?kx ?ky)))
(defrw2 const-iconv-and-const-pool (poolavg ?kx ?ky 1 1 same (iconv ?kx ?ky)) (cpool ?kx ?ky))
(defrw identity-kernel (conv2d 1 1 same none ?x (iconv ?kx ?ky)) ?x)
...
(defrw2 concatenation-and-conv.-2 (conv2d ?sx ?sy ?p none (concat 1 ?x ?z) (concat 1 ?y ?w)) (ewadd (conv2d ?sx ?sy ?p none ?x ?y) (conv2d ?sx ?sy ?p none ?z ?w)))
...
(defun resnext-block (input stride-h stride-w out-channels input-dim groups)
  (let* ((w1 (make-term (list 'weight out-channels input-dim 1 1)))
         (tmp (make-term (list 'conv2d 1 1 'same 'relu input w1)))
         (w2 (make-term (list 'weight out-channels (floor out-channels groups) 3 3)))
         (tmp (make-term (list 'conv2d stride-h stride-w 'same 'none tmp w2)))
         (w3 (make-term (list (* out-channels 2) out-channels 1 1)))
         (tmp (make-term (list 'conv2d 1 1 'same 'none tmp w3))))
    (when (or (> stride-h 1) (not (= input-dim (* out-channels 2))))
      (let ((w4 (make-term (list (* out-channels 2) input-dim 1 1))))
        (setq input (make-term (list 'conv2d stride-h stride-w 'same 'none input w4)))))
    (make-term (list 'relu (make-term (list 'add input tmp))))))

(defun resnext-50 ()
  (let* ((input (make-term (list 'input 1 3 224 224)))
         (weight (make-term (list 'weight 64 3 7 7)))
         (tmp (make-term (list 'conv2d 2 2 'same 'relu input weight)))
         (tmp (make-term (list 'poolmax 3 3 2 2 'same tmp)))
         (groups 32)
         (input-dim 64))
