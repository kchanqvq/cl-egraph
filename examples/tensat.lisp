...
(defun attention (input input-dim-1)
  (flet ((make-heads ()
           (make-term `(transpose-nd (reshape (matmul ,input (weight ,input-dim-1 ,input-dim-1))
                                              64 16 64)
                                     1 0 2))))
    (let* ((q (make-heads))
           (k (make-heads))
           (v (make-heads))
           (output (make-term `(reshape (transpose-nd (matmul (matmul ,q ,k) ,v) 1 0 2) 64 1024))))
      (values (make-term `(matmul ,input (weight ,input-dim-1 ,input-dim-1))) output))))

(defun bert ()
  (let* ((seq-length 64)
         (hidden-dims 1024)
         (input (make-term (list 'input seq-length hidden-dims)))
         (tmp (make-term (list 'relu input)))
         (outputs nil))
    (loop repeat 8 do
      (multiple-value-bind (next-in output) (attention tmp hidden-dims)
        (setq tmp next-in)
        (push output outputs)))
    outputs))
