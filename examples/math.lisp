...
(def-test math.simplify-root ()
  (let* ((egraph (make-egraph :analyses (list (make-const-analysis))))
         (a (make-term egraph '(/ 1 (- (/ (+ 1 (sqrt five)) 2)
                                     (/ (- 1 (sqrt five)) 2))))))
    (egraph-rebuild egraph)
    (run-rewrites egraph
                  '(commute-add commute-mul assoc-add assoc-mul
                    ...
                    recip-mul-div)
                  :max-enodes 75000)
    (is (equal '(/ 1 (sqrt five)) (greedy-extract egraph a #'ast-size)))))

#+nil (def-test math.simplify-factor ()
  (let* ((egraph (make-egraph :analyses (list (make-const-analysis))))
         (a (make-term egraph '(* (+ x 3) (+ x 1)))))
    (egraph-rebuild egraph)
    (run-rewrites egraph
                  '(commute-add commute-mul assoc-add assoc-mul
                    sub-canon div-canon add-0 mul-0 mul-1 -add-0 -mul-1
                    ...
                    recip-mul-div)
                  :max-enodes 75000)
    (is (eq  (enode-find (make-term egraph '(+ (+ (* x x) (* 4 x)) 3)))
             (enode-find a)))))
