...
(defvar *shape-analysis*
  (make-analysis-info
   :make (lambda (fsym &rest args)
           (if args
               (list (car (get-analysis-data (car args) *shape-analysis*))
                     (cadr (get-analysis-data (cadr args) *shape-analysis*)))
...
(defvar *cost-analysis*
  (make-analysis-info
   :make (lambda (fsym &rest args)
           (if (eq fsym 'matmul)
               (+ (get-analysis-data (car args) *cost-analysis*)
                  (get-analysis-data (cadr args) *cost-analysis*)
                  (let ((mn (get-analysis-data (car args) *shape-analysis*))
                        (nk (get-analysis-data (cadr args) *shape-analysis*)))
